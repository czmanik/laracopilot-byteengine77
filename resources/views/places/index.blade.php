...
<div class="max-w-6xl mx-auto px-4 py-10">
    ...
    <div class="flex flex-wrap gap-3 mb-8">
        <a href="{{ route('places.index') }}" class="px-4 py-2 rounded-full text-sm font-medium {{ !request('type') ? 'bg-rose-600 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:border-rose-400' }}">
            Vše
        </a>
        @foreach($placeTypes as $type)
        <a href="{{ route('places.index', ['type' => $type->id]) }}" class="px-4 py-2 rounded-full text-sm font-medium {{ request('type') == $type->id ? 'bg-rose-600 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:border-rose-400' }}">
            {{ $type->name }}
        </a>
        @endforeach
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <a href="{{ route('places.show', $place->slug) }}" class="group bg-white rounded-2xl shadow hover:shadow-lg transition-all duration-300 overflow-hidden">
            <div class="h-48 bg-gradient-to-br from-rose-400 to-orange-400 relative overflow-hidden">
                ...
                @endif
                @if($place->placeType)
                <span class="absolute top-3 left-3 bg-white/90 text-rose-700 text-xs font-semibold px-2 py-1 rounded-full">
            ...
            <div class="p-5">
                ...
                <div class="mt-4 flex items-center justify-between">
                    <div class="flex gap-2">
                        @foreach($place->accessibilityOptions->take(3) as $acc)
                        <span class="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded">{{ $acc->name }}</span>
                        @endforeach
                    </div>
                    <span class="text-rose-600 text-sm font-medium group-hover:translate-x-1 transition-transform inline-block">Detail →</span>
                </div>
            </div>
        </a>
        @endforeach
