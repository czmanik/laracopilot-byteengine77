...
@section('content')
<div class="relative h-64 bg-gradient-to-br from-rose-700 to-orange-500 overflow-hidden">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div class="lg:col-span-2">
            ...
            <div class="mb-8">
                ...
                <div class="space-y-3">
                    @foreach($stage->programSlots->sortBy('starts_at') as $slot)
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-2xl shadow p-6">
                ...
                <ul class="space-y-3 text-sm">
                    ...
                    <li class="flex gap-2"><span class="text-rose-500">🕐</span><span>{{ $place->program_from->format('d.m. H:i') }} – {{ $place->program_to?->format('d.m. H:i') }}</span></li>
                    @endif
                    @if($place->website)
                    <li class="flex gap-2"><span>🌐</span><a href="{{ $place->website }}" target="_blank" class="text-rose-600 hover:underline">Webové stránky</a></li>
                    @endif
                    @if($place->facebook)
                    <li class="flex gap-2"><span>📘</span><a href="{{ $place->facebook }}" target="_blank" class="text-rose-600 hover:underline">Facebook</a></li>
                    @endif
                    @if($place->instagram)
                    <li class="flex gap-2"><span>📷</span><a href="{{ $place->instagram }}" target="_blank" class="text-rose-600 hover:underline">Instagram</a></li>
                    @endif
                    @if($place->organizer)
                    <li class="flex gap-2"><span>👤</span><span>{{ $place->organizer->name }}</span></li>
                    @endif
                    @if($place->event)
                    <li class="flex gap-2"><span>🎪</span><span>{{ $place->event->getTypeLabel() }}</span></li>
                    @endif
                </ul>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow overflow-hidden">
                ...
                <div class="p-3 text-sm text-gray-500">{{ $place->address }}</div>
            </div>
            @push('head')
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    L.marker([{{ $place->lat }}, {{ $place->lng }}]).addTo(dmap).bindPopup('{{ addslashes($place->name) }}').openPopup();
                });
            </script>
            @endpush
            @endif
